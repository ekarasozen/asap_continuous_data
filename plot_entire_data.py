...
t = std[0].stats.starttime # does this stay here or moved to second loop? does it matter?
std.trim((t), (t + (12*5*60))-std[0].stats.delta) # 
...
for i in range(len(std)):
    ...
    ax13.set_xlabel("time after %s [s]" % strs[i].stats.starttime)
    ax14.set_xlim(td[0], td[-1])
    ax14.set_ylim(freqs_ne[-1], freqs_ne[0])
    ax14.set_xlabel("time after %s [s]" % strs[i].stats.starttime)
fig3.autofmt_xdate()
plt.tight_layout()
plt.draw()
fig3.savefig(outpath + 'degraded_scaleograms.png', bbox_inches='tight')
fig4.autofmt_xdate()
plt.tight_layout()
plt.draw()
fig4.savefig(outpath + 'processed_scaleograms.png', bbox_inches='tight')
fig5.autofmt_xdate()
plt.tight_layout()
plt.draw()
fig5.savefig(outpath + 'residual_scaleograms.png', bbox_inches='tight')
fig6.autofmt_xdate()
plt.tight_layout()
plt.draw()
fig6.savefig(outpath + 'noise_estimate_scaleograms.png', bbox_inches='tight')
